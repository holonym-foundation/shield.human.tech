...
contract BridgeAndFuel is Ownable, ReentrancyGuard {
     ...
     */
    function bridgeWithFuel(BridgeParams calldata p, bytes calldata swapData) external nonReentrant {
        ...
        // 3. Compute actual Fee Juice received (delta, not total balance) & deposit
        bytes32 fuelKey;
        uint256 fuelIndex;
        uint256 fuelReceived;
        {
            fuelReceived = feeJuiceToken.balanceOf(address(this)) - fjBalBefore;
