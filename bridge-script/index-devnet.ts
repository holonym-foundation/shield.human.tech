...
logger.info(
`🏗️Deploying L1 ${tokenConfig.symbol} with decimals ${tokenConfig.decimals} token contract`,
)
...
logger.info(`🏗️Deploying L2 ${tokenConfig.symbol} token contract`)
...
logger.info(
`⛓️L1 Chain: ${chain.chainInfo.name || 'Unknown'} (ID: ${
...
logger.warn(
'⚠️Low L1 wallet balance! You may need more ETH for gas fees.',
)
}
} catch (error) {
...
logger.warn(
'⚠️No tokens were deployed successfully. Skipping bridge test.',
)
}
}
