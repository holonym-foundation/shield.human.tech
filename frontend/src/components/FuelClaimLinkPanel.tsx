...
import { LS_KEY_BRIDGE_DEPOSITS } from '@/hooks/bridge/bridgeUtils'
...
export function FuelClaimLinkPanel() {
  ...
  const shortRecipient =
    recipient.length > 14 ? `${recipient.slice(0, 8)}…${recipient.slice(-6)}` : recipient
  ...
  return (
    <div className='mx-5 mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm'>
      ...
      <p className='text-amber-800 mt-1'>
        Fee juice for <span className='font-mono'>{shortRecipient}</span> is on L1, but they need this link to claim
        it on L2. Send it to them through a trusted channel — anyone with the link can pay gas to submit the claim,
        but the funds always land at <span className='font-mono'>{shortRecipient}</span>.
      </p>
