...
export interface WitnessResult {
  leafIndex: string
  siblingPath: string[]
  /** The epoch number used for the witness (converted from block number). */
  epoch: bigint
}
...
export async function fetchL2PassportAttestation(
  ...
  const res = await api.post('/api/attestation/passport', { portalAddress, bridgeAddress })
  const data = res.data as { l2Signature: number[]; nonce: number; maxAmount: string; deadline: string }
  return { l2Signature: data.l2Signature, nonce: data.nonce, maxAmount: data.maxAmount, deadline: data.deadline }
}
...
export async function waitForBlockProven(params: {
  ...
  if (rollupAddress) {
    try {
      ...
      console.log('[L2→L1] Polling L1 Rollup for proven block (need block=', blockNumberForProof, ')...')
  ...
  if (!blockProven && !usedPoll) {
    console.log('[L2→L1] Using fixed', fixedFallbackMs / 60_000, 'min wait for block finalization...')
...
export async function executeL1Withdraw(params: {
  ...
  console.log('[L2→L1] Sending L1 withdraw tx (recipient=', l1Address, 'amount=', amount.toString(), 'epoch=', epoch.toString(), ')...')
  ...
  console.log('[L2→L1] L1 withdraw tx encoded, sending to portal:', portalAddress)
  ...
  console.log('[L2→L1] L1 withdraw tx sent:', l1WithdrawTxHash, '— waiting for confirmation...')
...
export async function validateAndCaptureBlocksL2(
  ...
  try {
    ...
    nodeInfoSnapshot = serializeNodeInfo(nodeInfoForTx) as Record<string, unknown>
  } catch (e) {
...
export async function encryptAndBackupWithdrawalNonce(params: {
  ...
  const {
    ...
    isPrivacyModeEnabled, l1BlockNumberBeforeTx, l2BlockNumberBeforeTx, nodeInfoSnapshot,
    selectedToken,
  } = params
  ...
  const postResponse = await api.post('/api/bridge/operations', {
    ...
    l2BlockNumberBeforeTx: l2BlockNumberBeforeTx != null ? String(l2BlockNumberBeforeTx) : undefined,
    ...
    tokenNameL2: selectedToken?.title ?? `Clean ${selectedToken?.pairedSymbol ?? 'USDC'}`,
  ...
  if (!operationId) {
    throw new Error('Withdrawal aborted: server did not return operationId. No funds at risk.')
  }
...
export async function executeBurnAndExit(params: {
  ...
  const { walletAdapter, l1Address, aztecAddress, amount, nonce, isPrivacyModeEnabled, attestation, passportAttestation } = params
  const userAddress = AztecAddress.fromString(aztecAddress)

  console.log('[L2→L1] Sending burn+exit tx:', isPrivacyModeEnabled ? 'PRIVATE' : 'PUBLIC', 'amount:', amount.toString(), 'recipient:', l1Address, 'hasPassport:', !!passportAttestation)
  ...
  if (isPrivacyModeEnabled) {
    ...
    const passportData = passportAttestation
      ? { max_amount: BigInt(passportAttestation.maxAmount), nonce: BigInt(passportAttestation.nonce), deadline: BigInt(passportAttestation.deadline), signature: passportAttestation.l2Signature }
      : { max_amount: 0n, nonce: 0n, deadline: 0n, signature: new Array(64).fill(0) }
  ...
  console.log('[L2→L1] L2 exit tx sent, hash:', l2TxHash, 'blockNumber:', l2BlockNumber)
...
export async function persistBurnReceiptAndPollBlock(params: {
  ...
  console.log('[L2→L1] PATCH l2TxHash →', { operationId, status: 'submitted', l2TxHash, currentStep: 2 })
  const l2TxHashPatchOk = await patchOperationWithRetry(operationId, {
    status: 'submitted',
    l2TxHash,
    l2TxUrl,
    currentStep: 2,
  ...
  console.log('[L2→L1] PATCH l2BlockNumber →', { operationId, l2BlockNumber: String(blockNumberForProof) })
  const blockPatchOk = await patchOperationWithRetry(operationId, {
...
export async function fetchNodeInfoAndComputeWitness(params: {
  ...
  console.log('[L2→L1] Node info: rollupVersion=', rollupVersion, 'blockNumberForProof=', blockNumberForProof)
  if (rollupVersion == null) {
    throw new Error('Rollup version not available from Aztec node. Cannot compute L2→L1 message leaf.')
  }
  ...
  console.log('[L2→L1] PATCH witness →', { operationId, status: 'ready', l2ToL1MessageIndex: leafIndex, siblingPathLen: siblingPath.length, currentStep: 3 })
  const witnessPatchOk = await patchOperationWithRetry(operationId, {
    status: 'ready',
  ...
  return { leafIndex, siblingPath, rollupAddress, epoch, witnessPatchOk }
}
